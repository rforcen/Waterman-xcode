...
public:
    WatermanPolyhedra(int rad) { // generate coords & faces of the convex hull
        coords = null;
        this.rad = rad;
        double[] dcoords = WatermanData(rad); //sqrt((double) rad));    // generate double coords from SQRT(rad)!!
        ConvexHull(dcoords);
    }
    ...
    double[] WatermanData(double radius) { // retrun total number of vertex (3*ncoords)
        ...
        if (radius > 350) return null; // max radius 20*20=400
        ...
        for (int i = 0; i < ntc; i++) crd[i] = coords[i];
        delete[] coords;
        return crd;
    }
    ...
    // wc={2:[[ [0,0,0],[1,1,1] ] ,[[0,1,2],[3,4,5,6]], 3:[[[0,0,0],[1,1,1]],[0,1,2]]}
    String toPython() {
        String s = "";
